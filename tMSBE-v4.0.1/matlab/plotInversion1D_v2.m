...
IO_inversion_plot=0; %Plots occupation numbers around found peaks
...
outKey ='/Volumes/SAMbackup/tMSBE-RCAV-data-2021/tMSBE-v3.8-RCAV25-1D-n2p5-noThresh-theta2-1280-1920-3200lam-spontEmis-wExpSBE-focus10-QWoutput/run/out__';
date='021321';
test='tMSBE-RCAV25-1D-n2p5-noThresh-theta2-1280-1920-3200lam-spontEmis-wExpSBE-focus10-QWoutput';
%outKey = '../run/out__';
...
init_ind=find((t-t(1))/ps>=15.1,1);
final_ind=find((t-t(1))/ps>=15.7,1);
...
t_freq=1.0*fs; %output slice frequency
...
Ne_p2_qw=Ne_p2_qw_real+1i*Ne_p2_qw_imag;
Nh_p2_qw=Nh_p2_qw_real+1i*Nh_p2_qw_imag;
...
tmp_fig=figure('Name', 'Abs(E)');
set(tmp_fig,'Position',POS);
plot(t/ps,abs(pulse_fp),t/ps,abs(pulse_fm),t/ps,abs(pulse_bp),t/ps,abs(pulse_bm))
xlim([t(1),t(end)]/ps);
lgd=legend('FP','FM','BP','BM');
legend('show');
set(lgd,'Location','northeast');
xlabel('t (ps)');
ylabel('|E_{abs}| (V/m)');
saveas(tmp_fig,[saveKey,'abs_ind_field.png']);
end
...
if IO_video==1
    points=points*ps;
    t_init=t(1);%points(1)*ps-250*fs;%t(1);%points(1)-250*fs;
    t_end=t(end)-100*fs;%-600*fs;%points(4)+390*fs;
    t_fast_lhs=200*fs;
    t_fast_rhs=200*fs;
    ...
    while t(slice)<t_end
        tmp=Inv_qw(slice,:);
        tmp_00=Inv_00_qw(slice,:);
        tmp_p2=2.0*real(Inv_p2_qw(slice,:));
        plot(k_QW6,tmp,k_QW6,tmp_00,k_QW6,tmp_p2);
        %ylim(XLIM);
        %set(gca,'YTICK',[XLIM(1),0,XLIM(2)]);
        xlim(KLIM);
        set(gca,'FontSize',50)
        lgd=legend('Total','Zero order','Higher order');
    ...
    if t(slice)>=points(ind)-t_fast_lhs && t(slice)<=points(ind)+t_fast_rhs
        ...
        text(0.6*KLIM(2),0.85*peakInv,txt);
        drawnow
