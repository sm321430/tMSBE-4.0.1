...
%
% Apr/2015 - changed line 130, otherwise tricontf fails when
%            highest contour level == highest value.
%            Also a few other changes.
% Feb/2021 - Got rid of a lot of ; and ,
...
for k=1:length(nv)
  ...
  if any(Fi) % If there are contours at this level....
    ...
    iz=find(zz(1,any(id))>=nv(k));    % CHange from > to >= Feb/2021
    x2(:,iz)=x2([2 1],iz);
    y2(:,iz)=y2([2 1],iz);
    ...
    % line segments together into long lines
      ...
      while any(iN)
       ...
       % iN=find( abs(x2(rev1,:)-x2(rev2,iN))<1e-7 & abs(y2(rev1,:)-y2(rev2,iN))<1e-7 & iZ,1);
        if any(iN)  % If yes...
...
if plotcall
 ...
 %  set(h,'ContourMatrix',CS);
 %  set(h,'Children',handles);
 %  handles=h;
